    ...
    return (
        <div className="flex justify-center items-center min-h-[calc(100vh-200px)] text-white">
            <div className="w-full max-w-2xl p-6 border border-gray-700 rounded-xl shadow-lg bg-gray-800">
                ...
                <p><strong>Payment method:</strong> {order.metodoPago}</p>
                ...
                <ul className="space-y-4 mt-2">
                    {order.productos.map((prod, i) => (
                        <li key={i} className="flex items-center gap-4 bg-gray-600 p-3 rounded-lg shadow-sm">
                            <img
                                src={prod.imagen}
                                alt={prod.nombre}
                            ...
                            />
                            <div>
                                <p className="font-medium">{prod.cantidad}x {prod.nombre}</p>
                                <p className="text-xl text-white">${prod.precioUnitario}</p>
                            </div>
                        </li>
                    ))}
                </ul>

                {['Zelle', 'Venmo'].includes(order.metodoPago) && (
                    <div className="mt-6 bg-gray-700 p-4 rounded-lg shadow-inner">
                        <h3 className="text-lg font-semibold mb-2">Subir comprobante de pago</h3>
                        <form onSubmit={(e) => {
                            e.preventDefault();
