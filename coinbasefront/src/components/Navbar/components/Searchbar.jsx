...
export function SearchBar() {
  ...
  return (
    <div className="flex items-center space-x-2">
      <div
        className={`relative hover:lg:w-[20rem] hover:w-[10rem] h-[2rem] flex items-start justify-end group transition-all shadow-md  ${
          isOpen ? "lg:w-[20rem] w-[10rem]" : "w-0"
        }`}
      >
        <button
          className="absolute -right-2 w-[2rem] h-[2rem] flex justify-center items-center bg-slate-900 group-hover:bg-slate-800 rounded-md"
          onClick={() => {
            if (value.length > 0) {
              setValue("");
            }
